...
class speedTester {
    ...
    public static function create() {return self::$obj?:self::$obj = new self();}
    ...
    public function setCountIterations($count) {$this->iterations = $count;}
    ...
    public function testFunctions($consoleMode = false) {
        ...
        foreach($this->functions as $i => $function) {
            ...
            for ($j=0; $j < $this->iterations; $j++) {
                ...
                for ($t = time(); $t == time(););
                for ($result[$i]['iterPerSec'], $t = time(); time() == $t; $result[$i]['iterPerSec']++) call_user_func_array($function, $params);
            }
            $result[$i] += [
        ...
        if(!$consoleMode) return $result;
            ...
            foreach($result[0] as $k => $v) {
                foreach(array_merge([$v], array_column($result, $k)) as $V) if(mb_strlen($V) > (isset($this->colLengths[$i])?$this->colLengths[$i]:0)) $this->colLengths[$i] = mb_strlen($V);
                $i++;
            }
            ...
            foreach($result as $row) {
                $this->drawString(array_values(array_filter(array_replace($this->columns, $row), function($k) use($columns) {return isset($columns[$k]);},ARRAY_FILTER_USE_KEY)));
                echo $line;
